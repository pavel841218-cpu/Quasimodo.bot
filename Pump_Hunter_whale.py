...
async def get_active_usdt_pairs(session):
    ...
    Фильтр традиционных/неактивных индексов BingX (NCS*).
    """
...
async def get_klines_1h(session, symbol, limit=60):
    """
    4. Ошибка code=104915 проглатывается без логов.
    7. БЕЗОПАСНАЯ сортировка свечей (dict и list).
    """
    ...
    try:
        async with session.get(url, params=params, timeout=8) as resp:
            ...
            # Безопасная извлекалка timestamp для любого формата свечи
            def extract_time(k):
                if isinstance(k, dict):
...
async def main_loop():
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            # Исправлено предупреждение DeprecationWarning для UTC

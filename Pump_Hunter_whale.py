...
# -------------------------------------------------------------
# ПРОВЕРКА СТАКАНА НА ГОТОВНОСТЬ К ПАМПУ
# -------------------------------------------------------------
async def check_orderbook_for_pump_readiness(session, symbol):
    """
    Ищет признаки подготовки пампа в стакане.
    Манипуляции НЕ блокируют, а УСИЛИВАЮТ сигнал.
    """
    ...
    try:
        async with session.get(url, params=params, timeout=aiohttp.ClientTimeout(total=3)) as resp:
            if resp.status != 200:
                return {"pump_ready": False, "confidence": 0, "signals": [], "reason": "no_data"}
            
            res = await resp.json()
            if res.get("code") != 0 or "data" not in res:
                return {"pump_ready": False, "confidence": 0, "signals": [], "reason": "api_error"}
            
            bids = res["data"].get("bids", [])
            asks = res["data"].get("asks", [])
            
            if not bids or not asks:
                return {"pump_ready": False, "confidence": 0, "signals": [], "reason": "empty_book"}
            
            total_bid = sum(float(b[0]) * float(b[1]) for b in bids)
            total_ask = sum(float(a[0]) * float(a[1]) for a in asks)
            
            if total_ask <= 0:
                return {"pump_ready": False, "confidence": 0, "signals": [], "reason": "zero_asks"}
            
            bid_ask_ratio = total_bid / total_ask
            
            max_bid_wall = max(float(b[0]) * float(b[1]) for b in bids)
            wall_dominance = max_bid_wall / total_bid if total_bid > 0 else 0
            
            max_ask_wall = max(float(a[0]) * float(a[1]) for a in asks)
            
            pump_signals = []
            confidence = 0
            
            # 1. Стенка на покупку >50% объёма
            if wall_dominance > 0.5 and max_bid_wall > 50_000:
                pump_signals.append(f"Стенка на покупку ${max_bid_wall:.0f} ({wall_dominance*100:.0f}%)")
                confidence += 25
            
            # 2. Тонкий стакан на продажу
            if total_ask < total_bid * 0.2 and total_ask < 100_000:
                pump_signals.append(f"Тонкие аски: ${total_ask:.0f}")
                confidence += 20
            
            # 3. Перекос в сторону покупок
            if bid_ask_ratio > 5.0:
                pump_signals.append(f"Перекос покупок x{bid_ask_ratio:.1f}")
                confidence += 20
            
            # 4. Нет стены продавцов
            if max_ask_wall < total_ask * 0.3:
                pump_signals.append("Нет стены продавцов")
                confidence += 15
            ...
            if spread_pct < 0.5:
                pump_signals.append(f"Узкий спред: {spread_pct:.2f}%")
                confidence += 10
            
            pump_ready = confidence >= 40
            ...
            return {
                ...
                "wall_dominance": round(wall_dominance * 100, 0),
        ...
        return {"pump_ready": False, "confidence": 0, "signals": [], "reason": "timeout"}
    except Exception:
        return {"pump_ready": False, "confidence": 0, "signals": [], "reason": "error"}


# -------------------------------------------------------------
# ОСНОВНОЙ АЛГОРИТМ v2.2
# -------------------------------------------------------------
...
def format_signal_message(symbol, signal):
    ...
    if ob and ob.get('pump_ready'):
        msg += f"\n📖 <b>Стакан (памп готов! +{ob['confidence']}%):</b>\n"
        for s in ob.get('signals', []):
            msg += f"• {s}\n"
...
async def process_symbol(session, symbol, semaphore, volume_cache, sent_signals):
    async with semaphore:
        try:
            ...
            if signal:
                # Проверка стакана на готовность к пампу
                orderbook = await check_orderbook_for_pump_readiness(session, symbol)
                
                if orderbook.get("pump_ready"):
                    signal["score"] = min(99, signal["score"] + 10)
                    signal["orderbook"] = orderbook
                    logging.info(f"📖 Стакан подтверждает памп: +10 к Score ({symbol})")
            ...
            logging.debug(f"Ошибка обработки {symbol}: {e}")
            return None
...
async def scanner_loop():
    logging.info("🚀 Сканер Accumulation→Expansion v2.2 запущен")
...
async def handle_ping(request):
    return web.Response(text="Accumulation→Expansion v2.2 Active", status=200)

...
async def get_active_usdt_pairs(session):
    """
    Получает ТОЛЬКО реально торгуемые крипто-пары USDT perpetual.
    Приводит символы BingX к стандартному формату 'BTC-USDT'.
    """
    ...
    try:
        async with session.get(url, timeout=10) as resp:
            ...
            for item in tickers:
                ...
                # Пропускаем акции/индексы BingX
                if raw_symbol.startswith("NCS"):
                    continue

                # Форматируем символ, если BingX прислал без дефиса (BTCUSDT -> BTC-USDT)
                ...
                # Игнорируем инструменты без торгов и нулевой ценой
            ...
            logging.info(f"✅ BingX: найдено активных крипто-пар USDT: {len(active_pairs)}")
            return active_pairs

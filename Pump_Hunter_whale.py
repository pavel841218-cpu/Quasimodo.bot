...
MIN_RVOL_WHALE = 2.5       # Порог в whale-окно
MIN_RVOL_NORMAL = 3.0      # Порог в обычное время
...
def parse_kline(k):
    """Поддержка dict и list из ответов BingX API."""
    try:
...
def get_effective_candle_bounds(o, h, l, c):
    """Срезает экстремальные тени свечи."""
    body_high = max(o, c)
    body_low = min(o, c)
...
def detect_u_w_dip(candles):
    """Оптимизированный поиск ямы без повторного разбора свечей."""
    if len(candles) < 6:
        return False, 0.0
...
def find_shelf_before_breakout(klines_1h):
    """Поиск консолидации (полки) в закрытых свечах."""
...
async def get_active_usdt_pairs(session):
    """Ticker API + Автоформатирование BTCUSDT -> BTC-USDT."""
    ...
    try:
        async with session.get(url, timeout=10) as resp:
            ...
            if data.get("code") != 0:
                logging.warning(f"❌ BingX ticker code={data.get('code')}")
                return []
            ...
            for item in tickers:
                ...
                if raw_symbol.startswith("NCS"):
                    continue

                if raw_symbol.endswith("USDT") and not raw_symbol.endswith("-USDT"):
                    clean_name = raw_symbol[:-4]
                    formatted_symbol = f"{clean_name}-USDT"
                else:
                ...
                active_pairs.append(formatted_symbol)
...
async def get_klines_1h(session, symbol, limit=60):
    """Безопасная сортировка свечей (dict и list)."""
    ...
    try:
        async with session.get(url, params=params, timeout=8) as resp:
            ...
            def extract_time(k):
                if isinstance(k, dict):
                    return int(k.get("time", k.get("timestamp", 0)))
                if isinstance(k, (list, tuple)):
                    return int(k[0])
                return 0

            candles = sorted(candles, key=extract_time)
            return candles
...
async def refresh_shelves_memory(session):
    """Сканирование рынка и запись полок в памяти."""
...
async def check_breakouts(session, is_whale_window=False):
    """Мониторинг пробоя найденных полок.
    🔧 is_whale_window=True → сниженный порог RVOL (усиленный поиск).
    """
    if not ACTIVE_SHELVES:
        return

    rvol_threshold = MIN_RVOL_WHALE if is_whale_window else MIN_RVOL_NORMAL
    mode_label = "🐳 WHALE" if is_whale_window else "📊 NORMAL"

    logging.info(f"[{mode_label}] Мониторинг {len(ACTIVE_SHELVES)} полок (RVOL ≥ {rvol_threshold}x)...")
    ...
    for symbol, shelf in list(ACTIVE_SHELVES.items()):
        try:
            ...
            is_volume_spike = rvol >= rvol_threshold
            ...
            if is_pressing_high and is_volume_spike:
                ...
                window_tag = "🐳 [WHALE WINDOW 49-56 мин]" if is_whale_window else ""

                msg = (
                    f"🚀 <b>[BREAKOUT ALERT] {clean_symbol}</b> {window_tag}\n\n"
                ...
                logging.info(f"🚀 СИГНАЛ ОТПРАВЛЕН [{mode_label}]: {clean_symbol} | RVOL={rvol:.2f}x")
...
async def main_loop():
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            ...
            # Обновление полок каждый час
            if curr_min == 0 and curr_hour != last_scanned_hour:
                await refresh_shelves_memory(session)
                last_scanned_hour = curr_hour

            # 🔧 Определяем режим: whale-окно или обычный
            is_whale = WHALE_WINDOW_START_MIN <= curr_min <= WHALE_WINDOW_END_MIN

            if is_whale:
                # Усиленный поиск каждую минуту
                await check_breakouts(session, is_whale_window=True)
                await asyncio.sleep(60)
            else:
                # Обычный поиск каждые 2 минуты
                await check_breakouts(session, is_whale_window=False)

...
SLIPPAGE_PCT = 0.003
MIN_RR_RATIO = 2.0
MIN_PROFIT_PCT = 1.0       # Минимальный профит до тейка (в %)
MAX_PRICE_DISTANCE = 0.4    # Макс. уход цены от входа в % (не догоняем ушедшие цены)
MAX_PATTERN_AGE = 5         # Паттерн должен сформироваться не дальше 5 свечей назад

SIGNAL_COOLDOWN_HOURS = 4   # Защита от спама повторными алертами
...
def detect_bullish_qm(df):
    ...
    # --- ФИЛЬТР 2: ДИСТАНЦИЯ ОТ ЦЕНЫ ВХОДА ---
...
async def self_ping(session):
    app_url = os.getenv("RENDER_EXTERNAL_URL", "https://quasimodo-bot-1.onrender.com")
    while True:
        try:
            await asyncio.sleep(600)  # Пингуем раз в 10 минут
            async with session.get(app_url, timeout=aiohttp.ClientTimeout(total=5)) as resp:
...
async def scan_market():
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        # Запуск асинхронного самопинга от ухода в сон
        asyncio.create_task(self_ping(session))

        await send_telegram(
            session,
            "🤖 <b>Точный сканер Quasimodo обновлен!</b>\n"
            f"📊 TF: {', '.join(TIMEFRAMES)} | RR >= 1:{MIN_RR_RATIO} | Включены фильтры свежести свечей."
        )
            ...
            try:
                ...
                for symbol in symbols:
                    ...
                    for tf in TIMEFRAMES:
                        ...
                        if signal:
                            # Блокировка повторов (по монете, таймфрейму и направлению)
